...
(* If size if <>0 so is h and w *)
Theorem h_pos x : x < size -> 0 < h.
Proof.
unfold size; case h; auto with arith.
intros HH; contradict HH; auto with arith.
Qed.

Theorem w_pos x : x < size -> 0 < w.
Proof.
unfold size; rewrite mult_comm; case w; auto with arith.
intros HH; contradict HH; auto with arith.
Qed.

(* The reference list [1; 2; ...; size], i.e elements that can
   be contained in a non-empty cell  *)
Definition ref_list := progression size 1.

(* The reference list is composed of unique element *)
Theorem ref_list_ulist : ulist ref_list.
Proof.
refine (progression_list _ _).
Qed.

(* The size of the reference list is the h * w *)
...
(* An element outside the ref_list that is used for empty cell *)
Definition out := 0.
...
(* Define a position as a pair of natural numbers *)
Inductive pos: Set := Pos (x: nat) (y: nat).
...
Theorem pos_test_anti_sym p1 p2 : 
...
Definition valid_pos (p: pos) := let (x,y) := p in x < size /\ y < size.

(* Turn a position into a jump *)
Definition pos2n (p: pos) := let (x, y) := p in x * size + y.
...
(* Find the next position in the order 
    (0, 0), (0, 1), .. (0, size-1), (1, 0), .., 
    (1, size-1), ... (size-1, size-1) 
 *)
Definition next (p: pos) := let (x,y) := p in 
...
Proof.
destruct p as (x, y); simpl; case (eq_nat (S y) size); simpl.
...
Proof.
exact (in_fold_map _ (fun x y => (x, y)) p ref_list indexes).
Qed.

(* Create the list of positions (x, y) such that 0 <= x < size and 0 <= y < size *)
Definition cross2 :=
  let p := indexes in
  let q := indexes in
  fold_right (fun x l => (map (fun y => (Pos x y)) q) ++ l) nil p.
...
Proof.
unfold get; rewrite jump_nil; auto.
Qed.
...
Theorem update_get p v l :
   pos2n p < length l -> get p (update p v l) = v .
Proof.
...
(***************************************************)
(*    Restrict till position p:                    *)
(*  same values till position p, out after         *)
(***************************************************)

Definition prestrict p := restrict out (pos2n p).

Theorem prestrict_0 l : 
  prestrict (Pos 0 0) l = mk_0 out (length l).
Proof.
unfold prestrict; simpl; apply restrict_0.
Qed.

Theorem prestrict_all p s : length s <= pos2n p -> prestrict p s = s.
Proof.
unfold prestrict; intros H; apply restrict_all; auto.
Qed.

Theorem prestrict_length p s : length (prestrict p s) = (length s).
Proof.
unfold prestrict; simpl; apply restrict_length.
Qed.

Theorem prestrict_update p s : pos2n (next p) <= length s ->
...
Theorem prestrict_get s p q : pos2n p < pos2n q ->
  get p (prestrict q s) = get p s.
Proof.
intro H; unfold get, prestrict.
repeat rewrite <- jump_nth.
apply restrict_nth; auto.
Qed.

Theorem prestrict_get_default s p q : pos2n q <= pos2n p ->
...
(******************************************************)
(*    Refine:                                         *)
(* A state refines another if only non-empty cells    *)
(* have been occupied                                 *)
(******************************************************)
...
(*******************************************************)
(*   A grid is empty if all its cells are not occupied *)
(*******************************************************)
...
Proof.
intro p; unfold empty; rewrite get_mk_0; apply out_not_in_refl.
Qed.
...
Theorem get_row x y s :
   y < size -> get (Pos x y) s = nth y (row x s) out.
Proof.
...
Theorem length_column j s : j < size ->
...
rewrite plus_comm; rewrite jump_add.
...
Definition rect i (l: list nat) := 
  take_and_jump w size h (jump (w * (modn i h) +  h * (div i  h) * size) l).

(* Relation between get and rect *)
Theorem get_rect x y s : x < size -> y < size ->
     get (Pos x y) s =
     nth (modn x h * w + modn y w) (rect (div x h * h + div y w) s) out.
Proof.
...
match goal with |- context [take_and_jump _ _ _ (jump ?X _)]
  => 
     replace (x * size + y) with (X + (modn x h * size + modn y w))
end.
2: apply sym_equal.
  ...
  intros U3 n; generalize h; elim n; simpl; auto with arith; 
   clear n H1 H2 U1.
    intros h1; case h1; simpl; auto.
      intros HH; contradict HH; auto with arith.
    intros n1 _ l.
    case (le_or_lt (length l) (modn y w)); intros H1.
...
Theorem length_rect i s : i < size ->
...
(***************************************************)
(*    A function that checks if a grid is a sudoku  *)
(***************************************************)

(* A function that checks that a predicate P holds for i smaller than n *)
Definition check_P (P: nat -> Prop) (P_dec: forall i, {P i} + {~ P i}) : forall n,
...
Proof.
case l1; case l2; case l3; simpl; auto.
...
Theorem lit_test_anti_sym l1 l2 : 
 ...
 *)
Fixpoint clauses_update (l: lit) (c: list lit) (cs: clauses): clauses :=
  match cs with
    nil => nil
  | (n , c1) :: cs1 =>
...
Definition anti_literals (l: lit) :=
  let c := l :: nil in
  let (p, v) := l in
  let (x, y) := p in
      clause_merge (lit_rm c (gen_row x v))
      (clause_merge (lit_rm c (gen_column y v))
      (clause_merge (lit_rm c (gen_rect ((div x h) * h + (div y w))  v))
      (lit_rm c (gen_cell p)))).

(* Auxillary function that updates the list of clauses cs with 
 ...
 *)
Fixpoint gen_init_clauses_aux (s: list nat) (p: pos) (cs: clauses) :
  clauses :=
  match s with 
   nil => cs
  | a :: s1 => 
    ...
    let ll := L p a in 
      if (In_dec eq_nat a ref_list) then
         let cs1 := clauses_update ll (anti_literals ll) cs in 
           gen_init_clauses_aux s1 p1 cs1 
      else gen_init_clauses_aux s1 p1 cs
  end.
...
(***************************************************)
(*  Algorithm that finds one solution and insures  *)
(*  that it is unique                              *)
(***************************************************)
...
Theorem length_clause_insert c cs : 
  length (clause_insert c cs) = S (length cs).
Proof.
unfold clause_insert; apply ocons_length.
Qed.
...
Proof.
unfold clauses_merge; apply add_length.
Qed.
...
Theorem lit_insert_in l1 l2 c :
  In l1 (lit_insert l2 c) <-> l1 = l2 \/ In l1 c.
Proof.
...
Proof.
revert l i z; unfold gen_row, indexes.
...
Proof.
revert l j z; unfold gen_column, indexes.
...
Theorem gen_rect_correct l i z : 
    i < size ->
    (In l (gen_rect i z) <-> 
    exists i1, exists j1, l = L (Pos (h * div i h + i1) (w * modn i h + j1)) z 
                          /\ i1 < h /\ j1 < w).
Proof.
destruct l as ((x, y), z1); unfold gen_rect; split; intros H1.
  generalize (fold_insert2 _ 
    (fun p => (L (shift p (h * div i h) (w * modn i h)) z))  _ _ H1).
...
rewrite H2.
match goal with |- In (L (Pos (?X + ?Y) (?Z + ?T)) ?U) ?V =>
  change (In (L (shift (Pos Y T) X Z) U) V) 
end.
apply fold_insert1 with (f := (fun p => (L (shift p (h * div i h) (w * modn i h)) z))); auto.
...
Proof.
unfold ordered_clause, lit_insert; intro H; apply insert_olist; auto.
...
Proof.
intros H1 H2; unfold ordered_clause, clause_merge;
...
intros a l Rec; apply lit_insert_ordered; auto.
Qed.
...
Proof.
intro H; generalize l1; elim l2; simpl; auto; clear l1 l2.
...
Theorem lit_is_in_correct l c : ordered_clause c ->
    if (lit_is_in l c)  then In l c else ~ In l c.
Proof.
intro H; unfold lit_is_in; apply is_in_correct; auto.
- exact lit_test_trans.
- intros; apply lit_test_anti_sym.
- exact lit_test_exact.
Qed.
...
Theorem anti_literals_in p v1 v2 : In v2 ref_list ->
  v2 <> v1 -> In (L p v2) (anti_literals (L p v1)).
Proof.
...
- split.
    ...
    case H2.
  case (gen_cell_correct (L (Pos x1 y1) v2) x1 y1); auto;
...
(* A valid lit *)
Definition valid_lit (l: lit) s := 
  let (p,v) := l in ~ In (get p s) ref_list /\ valid_pos p /\ In v ref_list.
...
Proof.
destruct l as (p, v); intros H (H0, (H1, H2)); split; auto.
contradict H0.
case H; intros _ (_, H3); rewrite <- H3; auto.
Qed.

(* Validity works the other way than refinement *)
Theorem valid_refine cs s1 s2 : 
  refine s1 s2 ->  valid cs s2 -> valid cs s1.
Proof.
...
Theorem valid_update p v c s : 
  valid_clause c s -> valid_pos p ->
    (forall v, In v ref_list -> ~ In (L p v) c) -> valid_clause c (update p v s).
Proof.
intros H H1 H2 l Hl.
generalize (H l Hl) Hl; clear H Hl.
case l; simpl.
intros p1 v1 (H3, (H4, H5)) H6; split; auto.
rewrite update_diff_get; auto.
intro tmp; subst; case (H2 v1); auto.
Qed.

Theorem lit_insert_valid l c s : 
  valid_lit l s -> valid_clause c s -> valid_clause (lit_insert l c) s.
Proof.
destruct l as (p, v); simpl.
...
intros H6; apply (H4 (L p1 v1) H6).
Qed.

Theorem clause_merge_valid c1 c2 s :
  valid_clause c1 s -> valid_clause c2 s -> 
...
Theorem clauses_update_valid p v c cs s : 
  valid_lit (L p v) s -> ordered cs -> ordered_clause c ->
    (forall v1, In v1 ref_list -> v1 <> v -> In (L p v1) c) ->
    valid cs s -> valid (clauses_update (L p v) c cs) (update p v s).
Proof.
intros (H1, (H2, H3)) H4 H5 H6; generalize H4; 
  elim cs; simpl; auto; clear cs H4.
  intros _ _ n1 c1 H7; case H7.
intros (n1, c1) cs Rec H4 H7.
generalize (lit_is_in_correct (L p v) c1);
  case (lit_is_in (L p v) c1); intros H.
  apply Rec; auto.
...
apply clause_insert_valid; auto.
  ...
  intros v1 HH; case (eq_nat v v1); intros H8; subst; auto.
    intros H8; case H; auto.
    case (lit_rm_in (L p v1) c c1); auto.
    intros tmp _; case tmp; auto.
  intros H9; case (lit_rm_in (L p v1) c c1); auto.
  intros tmp _; case tmp; auto.
apply Rec; auto.
...
Theorem valid_init_c s : 
  length s = size * size -> empty s -> valid init_c s.
Proof.
...
Definition lit_sat l s :=
  match l with L p z => get p s = z end.
...
Theorem clause_insert_sat L l s : 
  sat (clause_insert L l) s <-> clause_sat L s /\ sat l s.
Proof.
split.
intros H1; split.
apply (H1 (length L) L).
case (clause_insert_in (length L, L) L l); auto.
intros n1 v1 H2.
apply (H1 n1 v1).
case (clause_insert_in (n1, v1) L l); auto.
...
rewrite <- get_row in H3; auto.
  exists (L (Pos i j) z); split; auto.
    case (gen_row_correct (L (Pos i j) z) i z); intros _ tmp; apply tmp; clear tmp.
    exists j; split; auto.
    rewrite <- H0; auto.
...
rewrite <- get_column in H3; auto.
  exists (L (Pos i j) z); split; auto.
    case (gen_column_correct (L (Pos i j) z) j z); intros _ tmp; apply tmp; clear tmp.
    exists i; split; auto.
    rewrite <- Eq1; auto.
  red; apply sym_equal; auto.
...
split.
  ...
  rewrite get_rect in Hl2.
      assert (Eq: div (h * div i h + i1) h * h + div (w * modn i h + j1) w = i).
...
case (in_ex_nth _ z 0 (rect i s)); intros tmp _ ; case (tmp H1);
  clear tmp H1.
intros j (Hj1, Hj2).
exists (L (Pos (div i h * h + div j w) (modn i h * w  + modn j w)) z); simpl.
split.
  match goal with |- In ?l (gen_rect ?i ?z) =>
    case (gen_rect_correct l i z); auto; intros _ tmp; apply tmp; clear tmp
  end.
  exists (div j w); exists (modn j w); repeat split; auto with arith.
    eq_tac; auto.
    eq_tac; auto.
...
rewrite get_rect.
    eq_tac; auto; [idtac | eq_tac]; auto.
      ...
      rewrite (mult_comm (div i h)); rewrite (mult_comm (modn i h)); 
    ...
    rewrite (fun x y => div_is_0 (div x y)); auto with arith.
      rewrite (fun x y => div_is_0 (modn x y)); auto with arith.
      repeat rewrite plus_0_r; apply sym_equal; apply div_mod_correct; auto.
    apply div_lt; auto.
    rewrite (mult_comm w); auto with arith.
    rewrite length_rect in Hj1; auto.
...
Theorem init_c_sat s : length s = size * size ->
  (sat init_c s <-> 
...
Theorem rect_aux1 x y : x < size -> y < size  ->
     div x h * h + div y w < size.
Proof.
intros H H1; unfold size; rewrite (mult_comm h); apply mult_lt_plus.
  apply div_lt; auto.
apply div_lt; auto.
rewrite mult_comm; auto.
Qed.

Theorem rect_aux2 x1 x2 y1 y2 : y1 < size -> y2 < size ->
  modn x1 h * w + modn y1 w = modn x2 h * w + modn y2 w ->
...
assert (Eq1: modn x1 h = modn x2 h).
...
replace (div y1 w) with (div y2 w); auto.
  eq_tac; auto.
  apply plus_reg_l with (modn x1 h * w).
  pattern (modn x1 h) at 2; rewrite Eq1; auto.
apply plus_reg_l with (div x1 h * h).
pattern (div x1 h) at 1; rewrite Eq2; auto.
Qed.

Theorem anti_literals_sat p z s : 
  sudoku s -> valid_pos p -> get p s = z -> ~ clause_sat (anti_literals (L p z)) s.
Proof.
...
Proof.
intros (V1, (V2, (V3, V4))).
assert (H: valid_lit (L p z) s); auto.
  apply (V2 n (L p z :: c)); auto with datatypes.
split; auto.
  apply clauses_update_ordered; auto.
...
intros n2 c2; simpl; intros [Hn2 | Hn2].
  ...
  exists (L p (get p s1)); split; auto with datatypes.
  simpl; auto.
...
assert (H: valid_lit (L p z) s); auto.
  apply (V2 n (L p z :: c)); auto with datatypes.
assert (F1: ordered_clause (L p z :: c)); auto with datatypes.
  apply (V1 n); auto with datatypes.
assert (F2: valid_clause (L p z :: c) s); auto with datatypes.
...
intros s2 Hs2; split; intros HH1.
  ...
  intros n2 c2; simpl; intros [HH | HH].
    ...
    case (Heq2 n (L p z :: c)); auto with datatypes.
    simpl; intros l ([Hl1 | Hl1], Hl2); subst.
      ...
      case (F2 (L p z)); auto with datatypes.
      intros _ (tmp, _); auto.
    exists l; auto.
...
assert (H: valid_lit (L p1 z1) s); auto.
...
Theorem try_all_sat s n c cs f :
  valid ((n, c) :: cs) s -> length s = size * size -> ordered cs ->
    (forall s cs1,
...
Theorem try_just_one_sat s n c cs f :
  valid ((n, c) :: cs) s -> length s = size * size -> ordered cs ->
    (forall s cs1,
       ...
       (forall s2,
         refine s s2 -> sudoku s2 -> sat cs s2 -> refine s1 s2)
       | jMore s1 s2 => 
                    refine s s1 /\ sat cs s1 /\
                    refine s s2 /\ sat cs s2 /\
                    s1 <> s2
       end) ->
    match try_just_one s c cs f with
         jNone => forall s1, refine s s1 -> sudoku s1 -> ~ sat ((n, c) :: cs) s1
       | jOne s1 => refine s s1 /\  sat ((n, c) :: cs) s1 /\
                    (forall s2,
                      refine s s2 -> sudoku s2 -> sat ((n, c) :: cs) s2 
                         -> refine s1 s2)
       | jMore s1 s2 => 
                    refine s s1 /\ sat ((n, c) :: cs) s1 /\
                    refine s s2 /\ sat ((n, c) :: cs) s2 /\
                    s1 <> s2
    end.
Proof.
revert s n cs f; elim c; simpl; clear c; simpl; auto.
  intros s n cs f Hv Hl Ho Hf s1 _ _ HH.
  case (HH n nil); auto with datatypes; intros x (HH1, _); inversion HH1.
intros (p1, z1) c1 Rec s n cs f Hv Hl Ho Rec1.
set (s1 := update p1 z1 s).
set (cs1 := clauses_update (L p1 z1) (anti_literals (L p1 z1)) cs).
assert (F1:length cs1 <= length cs).
  apply length_clauses_update; auto.
assert (F2: length s1 = size * size).
  unfold s1; rewrite length_update; auto.
assert (F3: valid cs1 s1).
  apply clauses_update_valid; auto with datatypes.
        apply (Hv n (L p1 z1::c1)); auto with datatypes.
      apply anti_literals_ordered.
    apply anti_literals_in.
  intros n2 c2 Hnc2; apply (Hv n2 c2); auto with datatypes.
...
assert (F8: forall s2 : list nat, 
  ...
  apply clauses_update_sat_rev; auto.
    ...
    case H1s2; intros _ (_, HH); simpl; rewrite <- HH; auto.
    rewrite F6; auto.
  intros l1 H1l H2l; case (anti_literals_sat p1 z1 s2); auto.
    case H1s2; intros _ (_, HH); simpl; rewrite <- HH; auto.
    rewrite F6; auto.
  exists l1; auto.
...
Theorem init_c_sudoku s :
  length s = size * size -> (sat init_c s <-> sudoku s).
Proof.
generalize (ref_list_ulist); intros Eq1.
...
intros H; case (init_c_sat s); auto; intros HH0 HH1; split; intros H1.
  ...
  split; auto.
  repeat split; intros i U; apply permutation_sym; 
...
Theorem sudoku_refine_id s1 s2 :
  sudoku s1 -> refine s1 s2 -> s1 = s2.
Proof.
intros H (H1, (H2, H3)).
...
replace n with (pos2n (Pos (div n size) (modn n size))).
  ...
  assert (F1: div n size < size); auto with arith.
    ...
    rewrite <- H1; auto.
  assert (F2: modn n size < size); auto with arith.
    apply mod_lt; auto with arith.
