...
Theorem anti_literals_in: forall p v1 v2, In v2 ref_list ->
  v2 <> v1 -> In (L p v2) (anti_literals (L p v1)).
Proof.
intros (x1, y1) v1 v2 H H1.
...
intros p v c s (H1, (H2, H3)) H4 l1; case l1; simpl.
intros p1 v1 H5.
...
Theorem clause_merge_valid: forall c1 c2 s, 
...
Theorem lit_rm_valid: forall c1 c2 s, 
...
Theorem clause_insert_valid: forall c cs s,
...
Theorem clauses_merge_valid: forall cs1 cs2 s,
...
Theorem clauses_update_valid: forall p v c cs s, 
...
Proof.
intros p v c cs s (H1, (H2, H3)) H4 H5 H6; generalize H4; 
...
Theorem gen_init_clauses_valid: forall s, 
...
Theorem sat_refine: forall cs s1 s2, 
...
Theorem lit_rm_sat: forall l1 l2 s,
...
Theorem clause_insert_sat: forall L l s, 
...
Theorem clauses_merge_sat: forall cs1 cs2 s, 
...
Theorem clauses_update_sat: forall l c cs s,
...
Theorem clauses_update_sat_rev: forall l c cs s,
  ordered_clause c -> lit_sat l s -> 
...
Theorem gen_row_sat: forall i z s, 
...
Theorem gen_column_sat: forall j z s,
...
Theorem gen_rect_sat: forall i z s,
  length s = size * size -> i < size -> 
...
Theorem gen_cell_sat: forall p s, 
...
Theorem init_c_sat: forall s, length s = size * size ->
  (sat init_c s <-> 
...
Theorem anti_literals_sat: forall p z s, 
...
Theorem invariant_equiv: forall cs s1 s2, 
...
Theorem invariant_gen_init_clauses: forall s,
...
Theorem try_one_sat: forall s n c cs f, 
    invariant ((n, c) :: cs) s ->
    (forall s cs1,
...
Theorem find_one_aux_sat: forall n s cs,
  length cs <= length n -> 
    invariant cs s ->
...
Theorem try_all_olist: forall s c cs f, 
    (forall s cs1,
...
Theorem try_all_sat: forall s n c cs f,
...
Theorem find_all_aux_sat: forall n s cs,
...
Theorem try_just_one_sat: forall s n c cs f, 
  valid ((n, c) :: cs) s -> length s = size * size -> ordered cs ->
    (forall s cs1,
       length cs1 <= length cs -> length s = size * size -> valid cs1 s ->
         ordered cs1 ->
...
Proof.
intros s n c; generalize s n; elim c; simpl; clear s n c; simpl; auto.
